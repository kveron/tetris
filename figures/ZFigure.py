...
class ZFigure:
    ...
    def draw_figure(x, y, game_field, position):
        ...
        if position == 1:
            ...
            game_field[i][j + 2] = 8
            game_field[i - 1][j] = 8
            game_field[i - 1][j + 1] = 8
        elif position == 2:
            Rectangle.draw_rectangle_light_blue(x, y)
            Rectangle.draw_rectangle_light_blue(x, y - 30)
            Rectangle.draw_rectangle_light_blue(x + 30, y - 30)
            Rectangle.draw_rectangle_light_blue(x + 30, y - 60)
            game_field[i][j] = 8
            game_field[i - 1][j] = 8
            game_field[i - 1][j + 1] = 8
            game_field[i - 2][j + 1] = 8
    ...
    def can_turn(x, y, game_field, position):
        ...
        j = int(x / 30)
        if position == 1:
            if i > 2 and game_field[i][j] == 0 and game_field[i - 2][j + 1] == 0:
                position = 2
        elif position == 2:
