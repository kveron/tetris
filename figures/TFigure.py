...
class TFigure:
    ...
    def can_turn(x, y, game_field, position):
        ...
        j = int(x / 30)
        if position == 1:
            if i > 2 and game_field[i - 1][j] == 0 and game_field[i - 2][j + 1] == 0:
                position = 2
        elif position == 2:
            if j < 9 and game_field[i - 1][j + 2] == 0:
                position = 3
        elif position == 3:
            if i > 3 and game_field[i - 2][j + 1] == 0:
                position = 4
        elif position == 4:
            if game_field[i][j] == 0 and game_field[i][j + 2] == 0:
                position = 1
        return position
